...
## 2026-08-27 (UTC) — Deterministic number tokens from run seed

- **`MapPresets.CreateBoard` / `ExpandBoard`:** optional `int? seed` forwarded to `NumberTokenLibrary.AssignMissingTokens`; `GameController` passes `RunSeed` on construct, map select, and act expansion.
- **Tests:** `NumberTokenTests` same-seed replay + different-seed divergence. No v0.1 tag.
